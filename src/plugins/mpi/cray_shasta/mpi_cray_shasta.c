...
extern int mpi_p_slurmstepd_task(const mpi_plugin_task_info_t *job, char ***env)
{
	...
	env_array_overwrite_fmt(env, PALS_LOCAL_RANKID_ENV, "%u", job->ltaskid);
	...
	env_array_overwrite_fmt(env, PMI_JOBID_ENV, "%u", job->step_id.job_id);
	env_array_overwrite_fmt(env, PMI_LOCAL_RANK_ENV, "%u", job->ltaskid);
	env_array_overwrite_fmt(env, PMI_LOCAL_SIZE_ENV, "%u", job->ltasks);
	env_array_overwrite_fmt(env, PMI_RANK_ENV, "%u", job->gtaskid);
	env_array_overwrite_fmt(env, PMI_SIZE_ENV, "%u", job->ntasks);
	env_array_overwrite_fmt(env, PMI_UNIVERSE_SIZE_ENV, "%u", job->ntasks);
